<?php
// dashboard.php
session_start(); // เพิ่ม session ถ้าจำเป็น
...
$kpis = fetch_all($mysqli, "
    SELECT
        COALESCE((SELECT SUM(net_amount) FROM fact_sales WHERE date_key >= DATE_SUB(CURDATE(), INTERVAL 29 DAY)),0) AS sales_30d,
        COALESCE((SELECT SUM(quantity) FROM fact_sales WHERE date_key >= DATE_SUB(CURDATE(), INTERVAL 29 DAY)),0) AS qty_30d,
        COALESCE((SELECT COUNT(DISTINCT customer_id) FROM fact_sales WHERE date_key >= DATE_SUB(CURDATE(), INTERVAL 29 DAY)),0) AS buyers_30d
");
...
(() => {
  ...
  new Chart(document.getElementById('chartMonthly'), { type:'line', data:{labels, datasets:[{label:'ยอดขาย (฿)', data:values,tension:.25,fill:true}]}, options:{plugins:{legend:{labels:{color:'#e5e7eb'}}}, scales:{x:{ticks:{color:'#c7d2fe'},grid:{color:'rgba(255,255,255,.08)'}}, y:{ticks:{color:'#c7d2fe'},grid:{color:'rgba(255,255,255,.08)'}}}} });
})();
